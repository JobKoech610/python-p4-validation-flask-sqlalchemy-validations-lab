...
class TestPost:
    ...
    def test_requires_title(self):
        ...
        with app.app_context():
            with pytest.raises(ValueError):
                post = Post(content="Some content", category='Non-Fiction')
                db.session.add(post)
                db.session.commit()
    ...
    def test_content_too_short(self):
        ...
        with app.app_context():
            content_string = "This is content"
            with pytest.raises(ValueError):
                post = Post(title='Secret, Why I love programming.', content=content_string, category='Non-Fiction')
                db.session.add(post)
                db.session.commit()
    ...
    def test_clickbait(self):
        ...
        with app.app_context():
            content_string = "This is content" * 150
            with pytest.raises(ValueError):
                post = Post(title='Why I love clickbait programming.', content=content_string, category='Fiction')
                db.session.add(post)
                db.session.commit()
